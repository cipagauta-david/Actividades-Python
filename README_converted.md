...
7. El **Sistema** recibe la solicitud e invoca el proceso interno **`UC-23: Crear Ticket`**. Este proceso se encarga de:
	...
	6. Tras la creación exitosa del ticket, el **Sistema** dispara dos acciones en paralelo:
...
8. El **Sistema** redirige al **Cliente** a una página de confirmación mostrando un mensaje de éxito: "¡Gracias! Hemos recibido tu solicitud. Tu número de ticket es [ID del Ticket]. Recibirás una confirmación en tu correo electrónico en breve."
...
6. El **Sistema** invoca el proceso **`UC-23: Crear Ticket`**, que se encarga de:
	...
	5. Tras la creación exitosa del ticket, el **Sistema** dispara dos acciones en paralelo:
		6. Inicia el proceso en segundo plano para el análisis con IA (`RF-IA-001`)
		7. Envía un correo electrónico de acuse de recibo al **Cliente**, incluyendo el ID único del nuevo ticket (`RF-TICKET-006`)   
...
5. El **Sistema** puede, opcionalmente, abrir una pequeña ventana modal para que el agente añada una nota interna para el equipo de especialistas (ej. "Cliente muy molesto, requiere revisión de producto dañado")
...
10. El **Sistema** elimina el ticket de la cola de triaje del agente y lo hace visible en la cola de los Agentes Especialistas (`UI-03`)
...
2. El `Ticket` ya no es visible en la cola de triaje
3. El `Ticket` es ahora visible en la cola de trabajo de los Agentes Especialistas
...
1. **Generar Inteligencia de Negocio:** Crear metadatos estructurados que permitan generar informes, identificar
...
3. El **Sistema** recupera de la base de datos toda la información asociada al `id` del cliente del ticket actual, incluyendo:
	1. Todos los demás `Tickets` asociados a ese cliente  
	2.  Todas las `Ordenes` asociadas a ese cliente
4. El **Sistema** presenta una "Vista de Perfil de Cliente" unificada, organizada y fácil de consultar. Esta vista contiene como mínimo dos secciones principales:
	a. **Sección de Historial de Soporte:** Una lista cronológica inversa (del más reciente al más antiguo) de todos los
	...
	b. **Sección de Historial de Órdenes:** Una lista cronológica inversa de todas las órdenes del cliente. Cada elemento
...
5. El **Agente** puede interactuar con esta vista para obtener más detalles:
	a. Al hacer clic en un ticket del historial, el **Sistema** podría mostrar un resumen o abrir el historial completo
		de esa conversación en un modal o panel lateral
	b. Al hacer clic en una orden, el **Sistema** muestra los detalles completos de la misma, incluyendo los productos
...
1. **Optimizar la Eficiencia Operativa:** Facilitar el intercambio de evidencia crucial (fotos, facturas) para acelerar
...
3. El **Agente** redacta el contenido de la nota. * (Opcional) El agente escribe "@" seguido del nombre de otro agente. El sistema sugiere usuarios y el agente selecciona uno para mencionarlo 
1. El **Agente** confirma la acción haciendo clic en "Guardar Nota"
2. El **Sistema** valida el contenido (no vacío) y lo guarda en la base de datos asociado al ticket y al agente autor
3. El **Sistema** actualiza la línea de tiempo del ticket en tiempo real, mostrando la nueva nota con un estilo visual distintivo que la diferencia de los mensajes públicos *(Si hubo menciones)
4. El **Sistema** envía una notificación interna (in-app o email, según configuración) al agente mencionado 
...
4. El **Sistema** presenta el formulario de edición con los siguientes campos clave:
	1. `Nombre/Tipo`: Identificador del perfil (ej. "wismo_agent")
	2. `Prompt del Sistema`: El texto base que instruye al LLM sobre su rol y reglas
	3. `Modelo`: Selector del modelo LLM a usar (ej. GPT-4o, Claude 3.5 Sonnet)
	4. `Temperatura`: Valor numérico (0.0-1.0) para controlar la creatividad
	5. `Umbral de Confianza`: Valor mínimo (0.0-1.0) para considerar una sugerencia como válida
...
1. **Mejorar y Estandarizar la Experiencia del Cliente (CX):** Asegurar que las respuestas para consultas comunes sean
	...
	** ** 
		...
		* **Para Crear una nueva plantilla: **   
		i. Hace clic en el botón "Nueva Plantilla"
			ii. El **Sistema** presenta un formulario con los campos: `Nombre` (un identificador corto y descriptivo),
		...
		v. El **Sistema** valida que el nombre y el contenido no estén vacíos, guarda la nueva plantilla en la base de
...
1. El catálogo de plantillas disponible para los agentes está actualizado (creado, modificado o eliminado) y los
...
1. **Crear un Activo de Conocimiento Corporativo:** Centralizar y estructurar el conocimiento de la empresa en un
...
7. **Inmediatamente**, el **Sistema** dispara un trabajo en segundo plano (asíncrono) para la vectorización del nuevo contenido. Este trabajo:
	a. Divide el `Contenido` en fragmentos de texto (chunks)
	b. Para cada chunk, llama a la API de embeddings para obtener su vector numérico
	c. Almacena cada chunk de texto junto con su vector en la base de datos vectorial
	d. Una vez que el trabajo en segundo plano finaliza con éxito, el **Sistema** actualiza el estado del artículo a
		`Activo` 
...
9. Un **Proceso en Segundo Plano** (worker) toma el trabajo de la cola y comienza a procesar el archivo CSV fila por fila:
	a. Para cada fila, valida los datos (tipos, longitudes, formatos) 
		* Busca si ya existe una `Orden` con el mismo identificador único. Si existe, la actualiza; si no, la crea (lógica de `upsert`)
	b. Registra el resultado de cada fila (éxito o error)
	c. Al finalizar el procesamiento, el **Proceso en Segundo Plano** actualiza el estado del trabajo a "Completado" y guarda un resumen (ej. " 1500 filas procesadas: 1490 exitosas, 10 fallidas")
10. El **Sistema** notifica al **Administrador** sobre la finalización del trabajo (ej. mediante una notificación en la UI o un correo electrónico) con un enlace al resumen de la importación
...
4. El **Sistema** presenta un formulario con los campos específicos para esa integración. Por ejemplo, para Mailgun:
	a. Clave de API (campo de contraseña, requerido) 
		* Dominio de envío (requerido)
	b. Clave de firma del Webhook (campo de contraseña, requerido para `API-02`)
	c. URL del Webhook de entrada (campo de solo lectura, para que el admin la copie y pegue en Mailgun)
...
7. El **Sistema** realiza dos acciones:
	a. Primero, guarda la configuración en la base de datos de forma segura (los secretos deben estar encriptados en reposo)
	b. Segundo, utiliza las nuevas credenciales para realizar una llamada de prueba a la API del servicio externo (ej. intenta enviar un email de prueba a una dirección designada usando la API de Mailgun)
...
1. **Optimizar el Enrutamiento:** Permitir que un agente con conocimiento del equipo y de la carga de trabajo asigne un ticket directamente al especialista más adecuado, saltándose la cola general de los Agentes Especialistas
...
8. El **Sistema** valida la acción y actualiza el registro del `Ticket`:
	a. Cambia el campo `asignadoAUsuarioId` al ID del Agente Especialista seleccionado 
		* Actualiza el `estado` del ticket a `escalado_nivel_2` (o `abierto`, si ya estaba en ese nivel)
	b. Crea un registro en el `LogEvento` del ticket indicando: "Reasignado por [Agente de Triaje] a [Agente Especialista] el [Fecha/Hora]"
	c. Si se añadió una nota, la guarda como un `Mensaje` de tipo `interno` asociado al ticket
...
7. El **Sistema** muestra una notificación inmediata en la UI: "Tu exportación ha comenzado. Te notificaremos cuando el archivo esté listo para descargar."
	a. El trabajo en segundo plano consulta la base de datos con los filtros aplicados y procesa los resultados en un flujo (`stream`) para construir el archivo CSV, garantizando un bajo consumo de memoria (`NFR-PERF-001`)
	b. Una vez que el archivo se ha generado y almacenado temporalmente de forma segura, el **Sistema** envía una notificación al usuario (ej. un ícono en la barra de navegación y/o un correo electrónico)
8. El **Usuario** hace clic en la notificación o en el enlace proporcionado
9. El **Sistema** inicia la descarga del archivo CSV en el navegador del usuario
10. El **Sistema** crea un registro en el `LogEvento` para auditar la acción: "El usuario [Nombre de Usuario] exportó [Número] tickets con los filtros [Resumen de Filtros] el [Fecha/Hora]"
...
2. El **Sistema** identifica el rol del usuario (Agente, Administrador)
...
4. **Agente:** Métricas personales (mis tickets asignados, mi tiempo de respuesta, etc.)
5. **Administrador:** Métricas agregadas de su equipo y métricas individuales de los agentes bajo su cargo
6. **Administrador:** Métricas globales de todo el sistema
7. El **Sistema** presenta el dashboard, que contiene varios widgets visuales, tales como:
	a. **Tarjetas de KPIs:** Tickets Abiertos, Tickets Resueltos Hoy, Tiempo Medio de Primera Respuesta (FRT), Tiempo Medio de Resolución (TTR), Tasa de Resolución en Primer Contacto (FCR), Puntuación de Satisfacción del Cliente (CSAT)
	b. **Gráficos de Tendencia:** Un gráfico de líneas mostrando el volumen de tickets creados vs. resueltos en los últimos 7/30 días.

	c. **Listas de Prioridad:** Una lista de los tickets más antiguos sin resolver o aquellos que están a punto de incumplir su SLA
8. El **Usuario** puede interactuar con el dashboard para refinar la vista, por ejemplo, cambiando el rango de fechas (hoy, últimos 7 días, último mes)
9. Si el usuario es Administrador, puede filtrar las métricas por agente o por equipo
...
3. **Timestamp:** Fecha y hora exactas del evento (en UTC).
4. **Usuario:** El usuario que realizó la acción (o "Sistema" si fue una acción automática)
5. **Tipo de Evento:** Una acción descriptiva (ej. `LOGIN_SUCCESS`, `USER_CREATED`, `CONFIG_AGENT_UPDATED`, `TICKET_DELETED`)
6. **Entidad Afectada:** El tipo de objeto que fue modificado (ej. `Usuario`, `Ticket`, `ConfigAgente`)
7. **ID de la Entidad:** El identificador único del objeto afectado
8. El **Administrador** utiliza los controles de filtrado para acotar la búsqueda. Puede filtrar por:
	a. Rango de fechas 
		* Usuario específico
	b. Tipo de evento
9. El **Administrador** aplica los filtros y el **Sistema** actualiza la tabla para mostrar solo los eventos que coinciden con los criterios
10. El **Administrador** hace clic en una fila de un evento para ver más detalles
11. El **Sistema** muestra una vista detallada (ej. en un modal) con toda la información del evento, que puede incluir:
	a. Dirección IP de origen 
		* Agente de usuario (navegador).

	b. Datos completos del cambio (ej. un "diff" o los valores "antes" y "después" de una modificación de configuración)
...
4. **Si no existe**, crea un nuevo registro de `Cliente` con el `email` y el `nombre` proporcionados
5. **Si existe**, utiliza el `Cliente` encontrado
6. El **Sistema** crea un nuevo registro de `Ticket` en la base de datos, asociándolo al ID del `Cliente`, y establece su estado inicial como `pendiente_ia`
7. Si se proporcionó un ID de `Orden` en los datos de entrada, el **Sistema** busca la orden y, si la encuentra, la asocia al `Ticket`
8. El **Sistema** crea un registro de `Mensaje` con el contenido del `asunto` y `cuerpo`, asociándolo al `Ticket` recién creado
9. **Si hay archivos adjuntos:**
	a. Para cada archivo, el **Sistema** lo sube al proveedor de almacenamiento (Supabase Storage) a través de su API 
		* Tras una subida exitosa, el **Sistema** crea un registro `Archivo` en la base de datos con la URL de almacenamiento y los metadatos del archivo
	b. Asocia cada registro `Archivo` al `Mensaje` creado en el paso 6
10. El **Sistema** devuelve una confirmación de éxito (con el ID del nuevo ticket) al proceso que lo invocó
...
9. El **Sistema** aplica la lógica de escalado automático (`RF-IA-005`)
		10. **Si** se cumple alguna condición de escalado (ej. `escalate: true`, confianza baja, palabras clave sensibles, adjunto de imagen), el estado del ticket se cambia a `escalado_nivel_2`
		11. **Si no**, el estado del ticket se cambia a `ia_sugerido`
12. El **Sistema** emite un evento en tiempo real (`COM-02`) para notificar a las interfaces de usuario que el ticket ha sido actualizado y está listo para revisión
...
4. El **Sistema** ejecuta la lógica de enrutamiento:
	a. Busca en las cabeceras `In-Reply-To` o `References` un ID de mensaje conocido 
		* Si no lo encuentra, busca en el `Subject` un patrón de ID de ticket (ej. `[Ticket

		#ABC-123]`)
	b. **Si se encuentra una referencia a un ticket existente** (flujo alternativo `A 1` que se convierte en principal para `UC-03`):
		a. El **Sistema** invoca el caso de uso **`UC-03: Responder a un Ticket (Correo) Existente`**, pasándole los datos
			del correo
		b. **Si no se encuentra ninguna referencia a un ticket existente:**
